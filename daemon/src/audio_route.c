                                     ...
                                     gpointer user_data)
{
    ...
    if (current && successful) {
        ...
        case ROUTE_OPERATION_CAPTURE_CONFIGURED_DEFAULT: {
            ...
            g_free(configured);
            /* Selecting A2DP and moving the live streams is sufficient to
             * acquire its transport. Do not suspend the sink here: doing so
             * also tears down BlueZ AVRCP absolute-volume state, which makes
             * the AirPods Max Digital Crown stop controlling system volume. */
